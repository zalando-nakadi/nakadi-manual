...
Each compatibility mode supports a different set of changes, which are
...
1. Consumers using older schemas can safely read events validated by
   newer schemas.
2. Clients using newer schemas can safely read events validated by
   older schemas.
...
The following json-schema attributes are not unsupported:
...
properties in order for validation to succeed. I.e. events with
properties not declared in the schema will be rejected. For this
reason, producers should first update their schemas with new
...
read events generated by newer schemas. On the other hand, it's not
backward compatible, in the sense that reading older events using
newer schemas is not safe.

For example, if the consumer has been developed with schema version
`1.5.0` in mind, reading events from version `1.5.0` on is safe, for
example, version `1.6.0`. But reading event generated by older
schemas, like `1.4.0`, is not safe.
...
We desencourage the usage of `additionalProperties` entirely. The more
