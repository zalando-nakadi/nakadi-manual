...
The default compatibility mode forward compatible, but full
compatibility usage is definitely encouraged and default mode will
...
It's possible to change the compatibility mode from `forward` to
`compatible`, e.g. it's possible to make the schema validation more
strict but never more relaxed.
